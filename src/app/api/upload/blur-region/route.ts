import { NextResponse } from "next/server";
import { getSession } from "@/lib/auth";
import { blurImageRegions } from "@/lib/blur-region";
import { BlurRectError, parseBlurRects } from "@/lib/blur-rects";
import {
  cardObjectPath,
  encodeCardImage,
  encodeGalleryImage,
} from "@/lib/image-variants";
import { encodeMasterJpeg } from "@/lib/photo-jpeg";
import {
  createPhotoMasterId,
  masterObjectPathFromGalleryPath,
} from "@/lib/photo-master";
import {
  downloadPrivateMaster,
  uploadPrivateMasterBytes,
} from "@/lib/photo-master-store";
import {
  VEHICLE_PHOTOS_BUCKET,
  getSupabaseAdmin,
  hasSupabaseServiceRole,
  storagePathFromPublicUrl,
} from "@/lib/supabase";

export const runtime = "nodejs";
export const maxDuration = 30;

const MAX_BYTES = 20 * 1024 * 1024;

/**
 * Borra os retângulos que o admin marcou na foto e grava galeria + capa
 * novas no Storage. Não há detector nem chamada externa.
 */
export async function POST(request: Request) {
  const session = await getSession();
  if (!session) {
    return NextResponse.json({ error: "Não autorizado." }, { status: 401 });
  }

  if (!hasSupabaseServiceRole()) {
    return NextResponse.json(
      { error: "Upload indisponível: falta SUPABASE_SERVICE_ROLE_KEY." },
      { status: 503 },
    );
  }

  try {
    const body = (await request.json()) as { url?: unknown; rects?: unknown };
    const url = typeof body.url === "string" ? body.url.trim() : "";
    if (!isOwnPhotoUrl(url)) {
      return NextResponse.json(
        { error: "URL de foto inválida." },
        { status: 400 },
      );
    }

    const rects = parseBlurRects(body.rects);
    const original = await downloadPhoto(url);
    const sourcePath = storagePathFromPublicUrl(url);
    const masterPath = sourcePath
      ? masterObjectPathFromGalleryPath(sourcePath)
      : null;
    const master = masterPath ? await downloadPrivateMaster(masterPath) : null;
    // Retângulos são normalizados: borrar o master (se existir) mantém a alta
    // e não deixa a placa nítida no download do admin. O master antigo sai
    // quando a URL antiga é removida ao salvar o anúncio.
    const processed = await blurImageRegions(
      master ? Buffer.from(master) : original,
      rects,
    );
    const [gallery, card] = await Promise.all([
      encodeGalleryImage(processed),
      encodeCardImage(processed),
    ]);

    const id = createPhotoMasterId();
    const galleryPath = `${id}.${gallery.extension}`;
    const cardPath = cardObjectPath(galleryPath);
    const supabase = getSupabaseAdmin();

    const [galleryUpload, cardUpload] = await Promise.all([
      supabase.storage.from(VEHICLE_PHOTOS_BUCKET).upload(galleryPath, gallery.buffer, {
        contentType: gallery.contentType,
        upsert: false,
        cacheControl: "31536000",
      }),
      supabase.storage.from(VEHICLE_PHOTOS_BUCKET).upload(cardPath, card.buffer, {
        contentType: card.contentType,
        upsert: false,
        cacheControl: "31536000",
      }),
    ]);

    if (galleryUpload.error) {
      return NextResponse.json(
        { error: galleryUpload.error.message || "Falha ao salvar a foto borracha." },
        { status: 500 },
      );
    }

    const { data } = supabase.storage
      .from(VEHICLE_PHOTOS_BUCKET)
      .getPublicUrl(galleryPath);
    const thumb = supabase.storage.from(VEHICLE_PHOTOS_BUCKET).getPublicUrl(cardPath);

    try {
      const jpeg = await encodeMasterJpeg(new Uint8Array(processed));
      await uploadPrivateMasterBytes(id, jpeg);
    } catch (error) {
      console.warn("[upload/blur-region] master:", error);
    }

    return NextResponse.json({
      url: data.publicUrl,
      thumbnailUrl: cardUpload.error ? null : thumb.data.publicUrl,
    });
  } catch (error) {
    if (error instanceof BlurRectError) {
      return NextResponse.json({ error: error.message }, { status: 400 });
    }
    if (error instanceof SyntaxError) {
      return NextResponse.json({ error: "Pedido inválido." }, { status: 400 });
    }
    console.error("[upload/blur-region]", error);
    return NextResponse.json(
      { error: "Não foi possível borrar a região desta foto." },
      { status: 500 },
    );
  }
}

function isOwnPhotoUrl(url: string) {
  if (!storagePathFromPublicUrl(url)) return false;
  const allowed = process.env.NEXT_PUBLIC_SUPABASE_URL?.trim();
  if (!allowed) return false;
  try {
    const parsed = new URL(url);
    const base = new URL(allowed);
    return parsed.protocol === "https:" && parsed.host === base.host;
  } catch {
    return false;
  }
}

async function downloadPhoto(url: string) {
  const imageResponse = await fetch(url, {
    signal: AbortSignal.timeout(20_000),
    redirect: "error",
  });
  if (!imageResponse.ok) {
    throw new BlurRectError("Não foi possível baixar a foto para borrar.");
  }
  const length = Number(imageResponse.headers.get("content-length") || 0);
  if (length > MAX_BYTES) {
    throw new BlurRectError("A foto é grande demais para borrar.");
  }
  const original = Buffer.from(await imageResponse.arrayBuffer());
  if (original.byteLength > MAX_BYTES) {
    throw new BlurRectError("A foto é grande demais para borrar.");
  }
  return original;
}
