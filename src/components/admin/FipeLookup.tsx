...
export function FipeLookup({ category, initialFipePrice, onApply }: Props) {
  ...
  useEffect(() => {
    ...
    // Só recarrega marcas ao mudar tipo (carro/moto).
  ...
  useEffect(() => {
    ...
    // onApply é estável via callback do pai; evitamos re-fetch em loop.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="mb-4 space-y-3 border border-white/10 bg-ink/40 p-4">
      <div>
        <p className="font-display text-xs font-semibold uppercase tracking-wider text-cream">
          Consulta FIPE
        </p>
        <p className="mt-1 text-xs text-muted">
          Uso interno — preenche marca/modelo e mostra referência de preço. Não
