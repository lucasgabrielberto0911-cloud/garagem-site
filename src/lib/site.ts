...
export const WHATSAPP_MESSAGES = {
  ...
  vehicleFinance: (label: string, isMoto = false) =>
    `Olá! Gostaria de opções de financiamento para ${isMoto ? "a" : "o"} ${label}. O consultor calcula a parcela no WhatsApp — sem valor inventado no site.`,
  vehicleTrade: (label: string, isMoto = false) =>
    `Olá! Tenho interesse ${isMoto ? "na" : "no"} ${label} e gostaria de colocar meu veículo na troca.`,
  finance: `Olá! Vi o site da ${site.name} e quero simular financiamento. O consultor calcula a parcela no WhatsApp, no meu caso — sem valor inventado no site.`,
  similarFavorites:
